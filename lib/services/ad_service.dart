...
class AdService {
  ...
  static const String _androidBannerAdUnitId =
      'ca-app-pub-7624999264785512/8900511771';

  // Interstitial ad unit ID
  static const String _androidInterstitialAdUnitId =
      'ca-app-pub-7624999264785512/8540175852';
  ...
  Future<void> initialize() async {
    ...
    MobileAds.instance.updateRequestConfiguration(
      RequestConfiguration(
        testDeviceIds: [
          'D0FDC8F32A5D6D81AB3767DFBCFCBB5C'
        ], // Add the test device ID from logs
    ...
    try {
      ...
      // Enable verbose logging in debug mode - MOVED AFTER INITIALIZATION
      ...
      await InterstitialAd.load(
        ...
        adLoadCallback: InterstitialAdLoadCallback(
          onAdLoaded: (InterstitialAd ad) {
            ...
            _interstitialAd?.fullScreenContentCallback =
              ...
              onAdFailedToShowFullScreenContent:
                ...
                debugPrint(
                    'Ad failed to show fullscreen content: ${error.message}');
                _isInterstitialAdLoading = false;
                ad.dispose();
                _interstitialAd = null;
          ...
          onAdFailedToLoad: (LoadAdError error) {
            ...
            // Retry after delay
            Future.delayed(
                Duration(seconds: retryDelay), () => preloadInterstitialAd());
          },
        ),
      );
    } catch (e) {
  ...
  Future<bool> showInterstitialAd() async {
    ...
    try {
      ...
      _interstitialAd =
          null; // Set to null since the ad will be disposed in callback
      return true;
    } catch (e) {
...
class _BannerAdWidgetState extends State<BannerAdWidget> {
  ...
  void _loadBannerAd() {
    ...
    debugPrint(
        '🔍 Attempting to load banner ad with ID: ${isTestAd ? "TEST_AD" : widget.adUnitId} for placement: ${widget.adPlacement}');
    ...
    _bannerAd = BannerAd(
      ...
      listener: BannerAdListener(
        onAdLoaded: (_) {
          debugPrint(
              '✅ Banner ad loaded successfully for placement: ${widget.adPlacement}');
          if (mounted) {
            setState(() {
              _isAdLoaded = true;
        ...
        onAdFailedToLoad: (ad, error) {
          debugPrint(
              '❌ Banner ad failed to load for placement: ${widget.adPlacement} - (${error.code}) ${error.message}');
          ad.dispose();
          if (mounted) {
            setState(() {
              _isAdLoaded = false;
          ...
          if (_retryAttempt < 5) {
            int retryDelay = math.min(30 * math.pow(2, _retryAttempt).toInt(),
                300); // Starts at 30s, caps at 5min

            // Shorter delay for network errors
            if (error.code == 1)
              retryDelay =
                  math.min(15 * math.pow(1.5, _retryAttempt).toInt(), 120);
        ...
        onAdOpened: (ad) {
          debugPrint(
              '👆 Banner ad opened for placement: ${widget.adPlacement}');
        },
        onAdClosed: (ad) {
          debugPrint(
              '👇 Banner ad closed for placement: ${widget.adPlacement}');
        },
      ),
    );
  ...
  Widget build(BuildContext context) {
    if (_bannerAd == null || !_isAdLoaded) {
      ...
      debugPrint(
          '🔍 Banner ad for ${widget.adPlacement} not ready to display yet. _bannerAd: ${_bannerAd != null}, _isAdLoaded: $_isAdLoaded');
      return Container(
    ...
    debugPrint('🔍 Displaying banner ad for ${widget.adPlacement}');
    return Container(
      width: widget.adSize.width.toDouble(),
      height: widget.adSize.height.toDouble(),
